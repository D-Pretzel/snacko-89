...
Worth testing on a phone with **PayPal not installed**, and on one with it installed. Without the app, both buttons fall back to the website and should behave exactly as they do on a desktop. With it, PayPal should open on a payment screen showing the amount — if it opens on the profile page with an empty form instead, the link is being shortened somewhere in between.
...
- **The PayPal link is written out as `paypal.com/paypalme/…`, not `paypal.me/…`.** The short form is a redirect to the long one, but on a phone the PayPal app claims the short domain and its handler keeps only the handle — the amount falls off the end and the customer lands on the profile page with an empty form. Pointing at the canonical URL skips the hop that loses it. The handle in the editor is still just the handle; only the link this page builds changed.
